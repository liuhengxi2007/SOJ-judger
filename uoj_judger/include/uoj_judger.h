...
struct RunLimit {
	...
	RunLimit(const double &_time, const int &_memory, const int &_output)
			: time(_time), memory(_memory), output(_output), real_time(0) {
	}
};
...
struct RunResult {
	...
	static RunResult from_file(const string &file_name) {
		...
		if (fres == NULL || fscanf(fres, "%d %lg %d %d", &res.type, &res.ust, &res.usm, &res.exit_code) != 4) {
...
struct RunCheckerResult {
	...
	static RunCheckerResult from_file(const string &file_name, const RunResult &rres) {
		...
		res.info = file_preview(file_name, 256);
		return res;
	}
...
void add_point_info(const PointInfo &info, bool update_tot_score = true) {
	...
	if (info.num < 0 && std::isnan(info.ust) && info.usm > 0) {
		details_out << "<test dnum=\"" << info.usm << "\"";
	} else {
...
struct RunProgramConfig {
	...
	string get_cmd() const {
		...
		sout << main_path << "/run/run_program"
			...
			<< " " << "--tl=" << (int)(limit.time * 1000.0)
			<< " " << "--rtl=" << (int)(limit.real_time * 1000.0)
			<< " " << "--ml=" << limit.memory
			<< " " << "--ol=" << limit.output
			<< " " << "--type=" << type
			<< " " << "--work-path=" << work_path;
...
RunValidatorResult run_validator(
	...
	if (ret.type != RS_AC || ret.exit_code != 0) {
		res.succeeded = false;
		res.info = file_preview(result_path + "/validator_error.txt", 256);
	} else {
		res.succeeded = true;
	}
	return res;
...
RunCompilerResult run_compiler(const char *path, ...) {
	...
	if (!res.succeeded) {
		if (ret.type == RS_AC) {
			res.info = file_preview(result_path + "/compiler_result.txt", 4096);
...
RunSimpleInteractionResult run_simple_interaction(
	...
	irpc.argv.push_back("/dev/null");
	irpc.argv.push_back(answer_file_name);

	irpc.limit.real_time = rpc.limit.real_time = rpc.limit.time + irpc.limit.time + 1.0;
...
RunCompilerResult compile_c(const string &name, const string &standard, const string &path = work_path) {
	return run_compiler(path.c_str(), 
			"/usr/bin/gcc", "-o", name.c_str(), "-x", "c", (name + ".code").c_str(), "-lm", "-O2", "-DONLINE_JUDGE", ("-std=c" + standard).c_str(), NULL);
}
RunCompilerResult compile_cpp(const string &name, const string &standard, const string &path = work_path) {
	return run_compiler(path.c_str(),
			"/usr/bin/g++", "-o", name.c_str(), "-x", "c++", (name + ".code").c_str(), "-lm", "-O2", "-DONLINE_JUDGE", 
			("-std=c++" + standard).c_str(), NULL);
}
...
RunCompilerResult compile(const char *name)  {
	...
	if (lang == "C++98") {
		return compile_cpp(name, "98");
	}
	if (lang == "C++11") {
		return compile_cpp(name, "11");
	}
	if (lang == "C++14") {
...
RunCompilerResult compile_c_with_implementer(const string &name, const string &standard, const string &path = work_path) {
	return run_compiler(path.c_str(), 
			"/usr/bin/gcc", "-o", name.c_str(), "implementer.c", "-x", "c", (name + ".code").c_str(), "-lm", "-O2", "-DONLINE_JUDGE", ("-std=c" + standard).c_str(), NULL);
}
...
RunCompilerResult compile_cpp_with_implementer(const string &name, const string &standard, const string &path = work_path) {
	return run_compiler(path.c_str(),
			"/usr/bin/g++", "-o", name.c_str(), "implementer.cpp", "-x", "c++", (name + ".code").c_str(), "-lm", "-O2", "-DONLINE_JUDGE", ("-std=c++" + standard).c_str(), NULL);
}
RunCompilerResult compile_with_implementer(const char *name)  {
	string lang = conf_str(string(name) + "_language");

	if (lang == "C++98") {
		return compile_cpp_with_implementer(name, "98");
	}
	if (lang == "C++11") {
		return compile_cpp_with_implementer(name, "11");
	}
	if (lang == "C++14") {
		return compile_cpp_with_implementer(name, "14");
	}
	if (lang == "C++17") {
